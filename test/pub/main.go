...
func thread(no int, wg *sync.WaitGroup, notify chan struct{}) {
	go func() {
		pc, err := client.NewPubClient("localhost", 8080, time.Second*5000, 1)
		if err != nil {
			log.Printf("%v\n", err)
			return
		}
...
func pub() {
	pc, err := client.NewPubClient("localhost", 12301, time.Second*5000, 1)
	if err != nil {
		log.Printf("%v\n", err)
		return
	}
...
func delay() {
	pc, err := client.NewPubClient("localhost", 12301, time.Second*50000, 1)
	if err != nil {
		log.Printf("%v\n", err)
		return
	}
